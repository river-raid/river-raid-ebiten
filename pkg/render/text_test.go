...
func TestRenderText_Position(t *testing.T) {
	...
	DrawText(img, []assets.TextSpan{
		{Text: "A", Row: 1, Col: 2, Ink: platform.ColorWhite},
	})

	wantInk := palette[platform.ColorWhite]
...
func TestRenderText_MultiCharAdvance(t *testing.T) {
	...
	DrawText(img, []assets.TextSpan{
		{Text: "AB", Row: 0, Col: 0, Ink: platform.ColorRed},
	})

	// "B" starts at pixel x=8. Row 1 of "B" is 0x42 (01000010) — bit 1 set at x=9.
	assertColor(t, img, 9, 1, palette[platform.ColorRed], "ink")
}
...
func TestRenderText_MultipleSpans(t *testing.T) {
	...
	DrawText(img, []assets.TextSpan{
		{Text: "A", Row: 0, Col: 0, Ink: platform.ColorRed},
		{Text: "B", Row: 1, Col: 1, Ink: platform.ColorGreen},
	})

	// Span 1: "A" at (0,0). Row 1 is 0x3c — bit 2 set at pixel (2, 1).
	assertColor(t, img, 2, 1, palette[platform.ColorRed], "span1 ink")

	// Span 2: "B" at row=1, col=1 → pixel origin (8, 8).
	// Row 1 of "B" is 0x42 (01000010) — bit 1 set at pixel (9, 9).
	assertColor(t, img, 9, 9, palette[platform.ColorGreen], "span2 ink")
}
...
func TestRenderText_UDGGlyph(t *testing.T) {
	...
	DrawText(img, []assets.TextSpan{
		{Text: string(assets.GlyphPlane), Row: 0, Col: 0, Ink: platform.ColorYellow},
	})

	wantInk := palette[platform.ColorYellow]
